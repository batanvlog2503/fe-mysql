...
const Computer = ({ onSelectComputer, computerSelected }) => {
  ...
  const loadComputers = async () => {
    try {
      ...
      alert("API computer isn't loaded")
    }
  }

  // Hàm để xác định class CSS dựa trên status
          ...
          <div className="col-12 table-computer">
            <div className="row computer-grid">
              {computers.map((computer, index) => (
                <button
                  key={index}
                  className={`computer-title col-xl-3 col-lg-3 col-sm-12 col-12 d-flex flex-column text-center ${getStatusClass(
                    computer?.status
                  )}`}
